...
namespace Wininit
{
    public static class M
    {
        ...
        public static void WriteS<T>(T value, IntPtr Address) where T : struct => G.Memory.WriteS(value, Address);
    ...
    public class QQMM
    {
        ...
        public QQMM(IntPtr hProcess, IntPtr lpBase)
        {
            Handle = hProcess;
            Base = lpBase;
        }
        ...
        public void WriteS<T>(T input, IntPtr Address)
        {
            int size = Marshal.SizeOf(input);
            byte[] arr = new byte[size];
        ...
        public byte[] Read(IntPtr address, int length)
        {
            ...
            //Console.WriteLine($"!RPM  {address.ToString("x2")} : {tempData.ToString()}");
            //if (!result)
