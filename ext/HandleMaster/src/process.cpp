...
namespace process
{
	class cpuz_driver * my_cpuz = new cpuz_driver();
  ...
  static process_context find_process_info(std::uint32_t pid,cpuz_driver& cpuz)
  {
  ...
  bool attach(std::uint32_t pid)
  {
    ...
    auto info = find_process_info(pid,*my_cpuz);
  ...
  bool read(PVOID base, PVOID buf, size_t len)
  {
   ...
   // auto& cpuz = cpuz_driver::instance();
    ...
    auto phys = my_cpuz->translate_linear_address(cur_context->dir_base, base);

    if(!phys)
      return false;

    return my_cpuz->read_physical_address(phys, buf, len);
  }
  ...
  bool write(PVOID base, PVOID buf, size_t len)
  {
    ...
    return my_cpuz->write_physical_address(phys, buf, len);
  }
